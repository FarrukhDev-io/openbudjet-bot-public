...
const bankApps = (card: string, amount: number) => [
  {
    name: "Click",
    url: `https://my.click.uz/services/p2p?card_number=${card}&amount=${amount}`,
  },
  {
    name: "Payme",
    url: `https://checkout.paycom.uz/card-to-card?to=${card}&amount=${amount * 100}`,
  },
  {
    name: "Uzum",
    url: `https://uzumbank.uz/transfer?card=${card}&amount=${amount}`,
  },
];
      ...
      <Card status={payment.status as any} className="flex flex-col gap-3 select-none">
        ...
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-100 leading-tight flex items-center gap-1.5">
        ...
        <div className="text-xs text-slate-400 flex flex-col gap-1.5">
          ...
          <p className="flex items-center gap-1.5">
            <FiHash className="text-slate-500 shrink-0" />
            <span className="text-slate-300">TG ID:</span>
            <code className="font-mono text-indigo-300 bg-slate-800/60 px-1 py-0.5 rounded">
              {payment.tg_id}
            </code>
          </p>
          ...
          <div className="flex justify-between items-center bg-slate-950/60 border border-slate-800/40 px-3 py-2 rounded-xl mt-1">
            <div className="flex items-center gap-2">
              <FiCreditCard className="text-slate-500 shrink-0" />
          ...
          {/* Masked card hint */}
          <p className="text-[10px] text-slate-500 pl-1">{maskCard(payment.card_number)}</p>
          ...
          <div className="flex justify-between items-center text-[10px] text-slate-500 mt-1">
            ...
            {payment.processed_at && (
              <span className="text-slate-600">
                → {payment.processed_at?.substring(0, 16)}
              </span>
            )}
          </div>
          ...
          {payment.status === "rejected" && payment.admin_note && (
            <div className="mt-1 px-3 py-2 bg-rose-500/10 border border-rose-500/30 rounded-xl text-rose-400 text-[11px] flex items-start gap-1.5">
              <FiAlertCircle className="mt-0.5 shrink-0" />
              <span className="italic">{payment.admin_note}</span>
            </div>
          )}
        </div>
